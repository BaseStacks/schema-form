...
const validationRuleKeys: (keyof ValidationRules)[] = ['required', 'minLength', 'maxLength', 'pattern', 'min', 'max', 'validate'];

export const getValidationProps = (field: FieldSchemaType<any>) => {
    const validationPropKeys = Object.keys(field).filter(key => (validationRuleKeys as string[]).includes(key));
